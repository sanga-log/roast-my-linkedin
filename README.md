...
미국에서 "Roast My Strava" 같은 AI 팩폭 서비스가 유행하는 걸 보고, 한국 LinkedIn 버전이 없길래 그냥 만들었다. 찾아봤더니 해외 비슷한 서비스들은 스크래핑 한도가 다 차거나 막혀서 작동을 안 하고 있었다. 그래서 한국스럽게 🇰🇷

<br>

## 주요 기능

- **🔗 URL 입력** → 8개 API 자동 fallback 스크래핑 → 즉시 분석
- **📄 PDF/스크린샷 업로드** → AI Vision 이미지 분석으로 팩폭
- **📊 오글 점수** 0~100점 게이지 + 허세 단어 뱃지 + 할미 충고 + 개선 팁
- **💬 피드백 시스템** → Slack Webhook으로 실시간 수신
...
```
① 랜딩
   └─ URL 입력 or PDF/스크린샷 업로드

② 스크래핑 (URL 입력 시)
   └─ 8개 RapidAPI 순차 시도
       ├─ 성공 → AI 분석으로 이동
       └─ 전부 실패 → "그럴 줄 알고 준비해뒀지 👵" → PDF/스크린샷 업로드 안내

③ AI 분석
   └─ Gemini 2.5 Flash가 충청도 할미로 변신
       └─ cringeScore, detectedWords, roastText, tips 구조화된 JSON 반환
...
`page.tsx` 하나에서 7개 화면을 상태 머신 패턴으로 관리한다. URL 라우팅 없이 순차적인 흐름을 단순하게 표현하기 위해 선택했다.
...
Vercel에 연결된 프로젝트라면 아래 명령어로 환경변수를 가져올 수 있다.

```bash
vercel env pull .env.local
```

직접 설정한다면 위 환경변수 표를 참고해 `.env.local` 파일을 생성한다.

### 배포

Vercel 대시보드에서 GitHub 레포를 연결하면 `main` 브랜치 push 시 자동 배포된다. **Settings → Environment Variables**에 필수 환경변수를 등록해야 정상 동작한다.
...
URL 입력 시 아래 8개 RapidAPI를 순서대로 시도한다. 하나가 성공하면 즉시 분석으로 넘어가고, 전부 실패하면 "그럴 줄 알고 준비해뒀지 👵" 화면으로 전환되어 PDF/스크린샷 업로드를 안내한다.

> LinkedIn은 봇 접근을 철저하게 막는다. 429(할당량 초과)든 403(차단)이든 동일하게 처리한다. 차단이 에러가 아니라 서비스 스토리가 되는 구조를 의도했다.
...
> 모든 API는 하나의 `RAPIDAPI_KEY`로 무료 구독하여 사용한다.

<br>

## AI 모델 선택 — Gemini 2.5 Flash

처음엔 Claude Sonnet 4를 썼다. 바이럴 서비스 특성상 트래픽을 예측할 수 없어서 비용 폭탄 위험이 있었고, Gemini 2.5 Flash로 전환했다.
...
`detectedWords`를 구조화된 배열로 받으면 결과 화면에서 뱃지로 렌더링할 수 있고, Slack 로그에도 그대로 기록된다.
...
에러는 Sentry로 수집하고, Sentry → Slack `#error-log` 채널로 자동 알림.
정상 사용 로그는 Slack Webhook으로 별도 처리한다.
...
DB나 어드민 패널 없이 Slack 하나로 활동 로그 + 피드백 + 에러 알림을 모두 처리한다.
1인 사이드 프로젝트에는 이 정도면 충분하다.
